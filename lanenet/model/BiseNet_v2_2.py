...
class StemBlock(nn.Module):
    def __init__(self):
        ...
        self.conv_1x1 = conv2d(16,32,1,0,1)
        self.conv_3x3 = conv2d(32,32,3,1,2)
        self.mpooling = nn.MaxPool2d(3,2,1)
        self.conv2 = conv2d(48,32,3,1,1)
    def forward(self,bottom):
...
class SemanticBranch(nn.Module):
    def __init__(self):
        ...
        self.stem = StemBlock()
        self.s3_ge1 = GatherExpansion(32,32,2)
...
class BinarySegmentationBranch(nn.Module):
    ...
    def __init__(self):
        ...
        if config.is_training==1:
            self.bsconv1_pre_help = conv2d(32,config.num_classes,1,0,1,use_rl=False,use_bn=True)#降维
        # 融合1/4 特征
        self.bsconv1_pre2 = conv2d(128,192,3,1,1,use_rl=True) #

        if config.is_training==1:
            self.bsconv1_pre2_help = conv2d(192,config.num_classes,1,0,1,use_rl=False,use_bn=True)#降维

        # 融合1/4 特征
        self.bsconv1_pre4 = conv2d((128+192-64),128,3,1,1,use_rl=True)#
        # 融合1/8 特征
        self.bsconv3 = conv2d(128,config.num_classes,1,0,1,use_rl=False,use_bn=True)

        if config.is_training==1:
            self.seghead1 = SegHead(32,32,config.num_classes)
            self.seghead3 = SegHead(32,32,config.num_classes)
            self.seghead4 = SegHead(64,64,config.num_classes)
            self.seghead5 = SegHead(128,128,config.num_classes)
    ...
    def forward(self,data,seg_stage_outputs,detail_stage_outputs):
        ...
        output_stage2_tensor = F.interpolate(out1, size=tuple(output_stage2_size), mode="bilinear",align_corners=True)
        if config.is_training==1:
            bsb_out1=self.bsconv1_pre_help(out1)
            bsb_out1 = F.interpolate(bsb_out1, size=tuple(out_put_tensor_size), mode="bilinear",align_corners=True)
        ...
        if config.is_training==1:
            bsb_out2=self.bsconv1_pre2_help(output_stage2_tensor)
            bsb_out2 = F.interpolate(bsb_out2, size=tuple(out_put_tensor_size), mode="bilinear",align_corners=True)
        ...
        # print(output_stage1_tensor.size())
        ...
        if config.is_training==1:
            sg_out1=self.seghead1(seg_stage_outputs['stg1'],size=out_put_tensor_size)
            sg_out3=self.seghead3(seg_stage_outputs['stg3'],size=out_put_tensor_size)
            sg_out4=self.seghead4(seg_stage_outputs['stg4'],size=out_put_tensor_size)
            sg_out5=self.seghead5(seg_stage_outputs['stg5'],size=out_put_tensor_size)
            return bsb_out,bsb_out1,bsb_out2,sg_out1,sg_out3,sg_out4,sg_out5
...
class BiSeNet(nn.Module):
    ...
    def forward(self,data,y=None):
        ...
        if config.is_training==1:
            bsb_out,bsb_out1,bsb_out2,sg_out1,sg_out3,sg_out4,sg_out5=self.binarySegmentationBranch(bga,sb["seg_stage_outputs"],db["detail_stage_outputs"])
            return {
                'instance_seg_logits': isb_res,
                # 'binary_seg_pred': binary_seg_ret,
                'binary_seg_logits': bsb_out,
                'bsb_out1': bsb_out1,
                'bsb_out2': bsb_out2,
                'sg_out1': sg_out1,
                'sg_out3': sg_out3,
        ...
        bsb_out=self.binarySegmentationBranch(bga,sb["seg_stage_outputs"],db["detail_stage_outputs"])
        return {
            'instance_seg_logits': isb_res,
            # 'binary_seg_pred': binary_seg_ret,
            'binary_seg_logits': bsb_out
...
if __name__ == '__main__':
    ...
    # print('BiSeNet_v2', output["bsb_out2"].size())
    # print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    print((0.11+0.12+0.13+0.14)/3)
