...
class SemanticBranch(nn.Module):
    ...
    def forward(self,bottom):
        ...
        if self.training:
            seghead1 = self.seghead1(stg1)
            seghead2 = self.seghead3(stg3)
            seghead3 = self.seghead4(stg4)
            seghead4 = self.seghead5(stg5)
...
class BinarySegmentationBranch(nn.Module):
    ...
    def __init__(self):
        ...
        self.bsconv1_pre = conv2d(128,128,3,1,1,use_rl=True) #
        # 融合1/2 特征
        self.bsconv1_pre2 = conv2d((128+(128-32)),32,3,1,1,use_rl=True) #
        # 融合1/4 特征
        self.bsconv1_pre4 = conv2d(128,16,3,1,1,use_rl=True)#
        # 融合1/8 特征
        self.bsconv3 = conv2d(16,config.num_classes,1,0,1,use_rl=False,use_bn=True)
...
class BiSeNet(nn.Module):
    def __init__(self):
        ...
        self.bga = BGA(128)
        if config.is_training==1:
            self.seghead = SegHead(128,128,config.num_classes)
    ...
    def forward(self,data,y=None):
        ...
        if config.is_training==1:
            aux1_loss = self.criterion(sb["seghead1"], y)
            aux2_loss = self.criterion(sb["seghead2"], y)
            aux3_loss = self.criterion(sb["seghead3"], y)
            aux4_loss = self.criterion(sb["seghead4"], y)
            return {
